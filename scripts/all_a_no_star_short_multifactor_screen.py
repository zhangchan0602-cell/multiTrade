...
- docs/list/all_a_no_star_short_multifactor_top5.csv
- docs/list/all_a_no_star_short_multifactor_top5.md
...
from all_a_no_star_mid_multifactor_screen import (
    HEADERS,
    KLINE_URL,
    OUTPUT_DIR,
...
def kline_get(params: Dict, timeout: int = 25):
    last_err = None
    for trust_env in (False, True):
        try:
            with requests.Session() as session:
                session.trust_env = trust_env
                return session.get(KLINE_URL, params=params, headers=HEADERS, timeout=timeout)
...
def get_short_kline_feature(code: str, retries: int = 6) -> Dict:
    ...
    for i in range(retries):
        try:
            r = kline_get(params=params, timeout=25)
            r.raise_for_status()
            js = r.json()
            ...
            t = np.array(turnovers, dtype=float)
            rets = c[1:] / c[:-1] - 1.0
...
def add_fast_prefilter_columns(df: pd.DataFrame, as_of) -> pd.DataFrame:
    ...
    df["fast_prefilter_pass"] = (
        (~df["is_st"])
        & (df["calendar_listed_days"].fillna(-1) >= 45)
...
def select_short_kline_candidates(df: pd.DataFrame, limit: int) -> List[str]:
    ...
    trade_prefilter = (
        ...
        & (df["calendar_listed_days"].fillna(-1) >= 90)
...
def write_outputs(scored: pd.DataFrame, merged: pd.DataFrame, run_ts: datetime) -> None:
    ...
    scored[export_cols].to_csv(
        OUTPUT_DIR / "all_a_no_star_short_multifactor_passed.csv", index=False, encoding="utf-8-sig"
    )

    with (OUTPUT_DIR / "all_a_no_star_short_multifactor_passed.md").open("w", encoding="utf-8") as f:
        f.write("# 全A（不含科创板）短线多因子模型 符合清单\n\n")
        f.write(f"- 生成时间: {run_ts.strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    top5[export_cols].to_csv(
        OUTPUT_DIR / "all_a_no_star_short_multifactor_top5.csv", index=False, encoding="utf-8-sig"
    )

    with (OUTPUT_DIR / "all_a_no_star_short_multifactor_top5.md").open("w", encoding="utf-8") as f:
        write_rank_table(f, top5, "全A（不含科创板）短线多因子模型 Top 5", run_ts)

    top20 = scored.head(20).copy()
    top20[export_cols].to_csv(
        OUTPUT_DIR / "all_a_no_star_short_multifactor_top20.csv", index=False, encoding="utf-8-sig"
    )

    with (OUTPUT_DIR / "all_a_no_star_short_multifactor_top20.md").open("w", encoding="utf-8") as f:
        write_rank_table(f, top20, "全A（不含科创板）短线多因子模型 Top 20", run_ts)
    ...
    with (OUTPUT_DIR / "all_a_no_star_short_multifactor_summary.md").open("w", encoding="utf-8") as f:
        f.write("# 全A（不含科创板）短线多因子筛选统计\n\n")
        f.write(f"- 生成时间: {run_ts.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        f.write("## 输出文件\n\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_passed.csv`\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_passed.md`\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_top5.csv`\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_top5.md`\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_top20.csv`\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_top20.md`\n")
        f.write("- `docs/list/all_a_no_star_short_multifactor_summary.md`\n\n")
        f.write("## 口径说明\n\n")
        ...
        f.write("- 交易目标: 盘后运行，次日择机买入Top5，目标持有后续2-3个交易日\n")
...
def main() -> None:
    ...
    calendar_days = df["calendar_listed_days"]
