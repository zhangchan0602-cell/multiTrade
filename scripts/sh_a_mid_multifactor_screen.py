...
输出文件：
- docs/list/sh_a_mid_multifactor_passed.csv
- docs/list/sh_a_mid_multifactor_passed.md
- docs/list/sh_a_mid_multifactor_top30.md
- docs/list/sh_a_mid_multifactor_summary.md
"""
...
SH_FILTER = '(TRADE_MARKET_CODE in ("069001001001","069001001006"))(LISTING_STATE="0")(SECURITY_TYPE_CODE="058001001")'
CODE_RE = re.compile(r"^6\d{5}$")
...
def safe_get_json(params: Dict, retries: int = 6, sleep_s: float = 0.7) -> Dict:
    ...
    for i in range(retries):
        try:
            r = requests.get(DATA_CENTER_URL, params=params, headers=HEADERS, timeout=25)
            r.raise_for_status()
            js = r.json()
...
def fetch_sh_quotes() -> pd.DataFrame:
    rows = []
    page = 1
    while True:
        params = {
    ...
    if df.empty:
        raise RuntimeError("empty SH quote dataset")
...
def fetch_latest_finance(secucodes: List[str], as_of: date) -> pd.DataFrame:
    ...
    if not frames:
        raise RuntimeError("empty finance dataset for SH universe")
...
def get_kline_feature(code: str, retries: int = 4) -> Dict:
    ...
    for i in range(retries):
        try:
            r = requests.get(KLINE_URL, params=params, headers=HEADERS, timeout=25)
            r.raise_for_status()
            js = r.json()
...
def write_outputs(scored: pd.DataFrame, merged: pd.DataFrame, run_ts: datetime) -> None:
    ...
    scored[export_cols].to_csv(OUTPUT_DIR / "sh_a_mid_multifactor_passed.csv", index=False, encoding="utf-8-sig")

    # full markdown list
    with (OUTPUT_DIR / "sh_a_mid_multifactor_passed.md").open("w", encoding="utf-8") as f:
        f.write("# 沪A中线多因子模型 符合清单\n\n")
        f.write(f"- 生成时间: {run_ts.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"- 共 {len(scored)} 只\n\n")
        f.write("| 排名 | 代码 | 名称 | 行业 | 总分 |\n")
        f.write("|---:|---:|---|---|---:|\n")
        for _, r in scored.iterrows():
            f.write(
                f"| {int(r['rank'])} | {r['code']} | {r['name']} | {r['industry']} | {float(r['score']):.4f} |\n"
            )

    # top30
    top30 = scored.head(30)
    with (OUTPUT_DIR / "sh_a_mid_multifactor_top30.md").open("w", encoding="utf-8") as f:
        f.write("# 沪A中线多因子模型 Top 30\n\n")
        f.write(f"- 生成时间: {run_ts.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("| 排名 | 代码 | 名称 | 行业 | 总分 | 价值 | 质量 | 成长 | 动量 | 低波 |\n")
        f.write("|---:|---:|---|---|---:|---:|---:|---:|---:|---:|\n")
        for _, r in top30.iterrows():
            f.write(
                "| {rank} | {code} | {name} | {industry} | {score:.4f} | {value:.4f} | {quality:.4f} | {growth:.4f} | {mom:.4f} | {lowvol:.4f} |\n".format(
                    rank=int(r["rank"]),
                    code=r["code"],
                    name=r["name"],
                    industry=r["industry"],
    ...
    with (OUTPUT_DIR / "sh_a_mid_multifactor_summary.md").open("w", encoding="utf-8") as f:
        f.write("# 沪A中线多因子筛选统计\n\n")
        f.write(f"- 生成时间: {run_ts.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"- 沪A初始样本: {len(merged)}\n")
        f.write(f"- K线成功样本: {kline_ok}\n")
        f.write(f"- 硬过滤后样本: {int(merged['hard_pass'].sum())}\n")
        f.write(f"- 最终符合样本: {len(scored)}\n\n")
        f.write("## 输出文件\n\n")
        f.write("- `docs/list/sh_a_mid_multifactor_passed.csv`\n")
        f.write("- `docs/list/sh_a_mid_multifactor_passed.md`\n")
        f.write("- `docs/list/sh_a_mid_multifactor_top30.md`\n")
        f.write("- `docs/list/sh_a_mid_multifactor_summary.md`\n\n")
        f.write("## 口径说明\n\n")
        f.write("- 股票池: 沪A（上交所主板 + 科创板）\n")
...
def main() -> None:
    ...
    print("[1/4] fetch SH quote universe...")
    quote = fetch_sh_quotes()
