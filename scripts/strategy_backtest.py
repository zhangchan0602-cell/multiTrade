...
def _get_snap(trade_date: str) -> pd.DataFrame:
    """带内存缓存的行情快照，含 ts_code / close / pct_chg。"""
    if trade_date not in _snap_cache:
        df = fetch_daily_snapshot(trade_date)
        if df is None or df.empty:
            _snap_cache[trade_date] = pd.DataFrame(columns=["ts_code", "close", "pct_chg"])
        else:
            for col in ("close", "pct_chg"):
                df[col] = pd.to_numeric(df[col], errors="coerce")
            _snap_cache[trade_date] = df[["ts_code", "close", "pct_chg"]].copy()
...
def main() -> None:
    ...
    parser.add_argument("--strategy",      default="all",   help="rps90|short|tail|all（默认 all）")
    ...
    strategies = (
        ["rps90", "short", "tail"] if args.strategy == "all"
        else [args.strategy]
    )
    ...
    if all_results:
        ...
        if len(all_results) > 1:
            print(f"\n三策略横向对比:")
            label_map = {"rps90": "RPS双90", "short": "短线盘后版", "tail": "短线尾盘版"}
